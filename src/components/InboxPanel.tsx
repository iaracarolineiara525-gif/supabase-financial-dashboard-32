...
export function InboxPanel() {
  ...
  const loadThread = async (conversationId: string, forceMediaRefresh = false) => {
    ...
    setMessages((current) => {
      if (forceMediaRefresh) return nextMessages;
      const currentById = new Map(current.map((message) => [message.id, message]));
      const now = Date.now();
      return nextMessages.map((message) => {
        const previous = currentById.get(message.id);
        const previousExpiry = previous?.media_url_expires_at ? new Date(previous.media_url_expires_at).getTime() : 0;
        return previous?.media_url && previous.media_storage_path === message.media_storage_path && previousExpiry > now + 60_000
          ? { ...message, media_url: previous.media_url, media_url_expires_at: previous.media_url_expires_at }
          : message;
      });
    });
  ...
  useEffect(() => {
    ...
    const expiries = messages
      .map((message) => message.media_url_expires_at ? new Date(message.media_url_expires_at).getTime() : 0)
      .filter((value) => Number.isFinite(value) && value > 0);
    if (expiries.length === 0) return;
    const delay = Math.max(1_000, Math.min(...expiries) - Date.now() - 30_000);
    const timer = window.setTimeout(() => void loadThread(selectedId, true), delay);
    ...
    <Card className="glass-card overflow-hidden rounded-[1.25rem]">
      <CardHeader className="flex flex-col gap-3 border-b border-border/60 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">Organize respostas, não lidas e janela de atendimento de 24 horas com histórico registrado.</p>
        </div>
        <Button variant="outline" size="sm" className="w-full rounded-lg bg-transparent sm:w-auto" onClick={() => void loadConversations()} disabled={isLoading}>
          <RefreshCw className={cn("mr-2 h-3.5 w-3.5", isLoading && "animate-spin")} /> Atualizar caixa de entrada
        </Button>
      </CardHeader>
      <CardContent className="grid min-h-[28rem] gap-3 p-3 sm:p-4 lg:grid-cols-[minmax(230px,0.72fr)_minmax(0,1.28fr)]">
        ...
        <div className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-border/60 bg-background/30">
          {!selectedConversation ? <div className="flex min-h-[20rem] flex-1 flex-col items-center justify-center p-6 text-center"><div className="mb-3 rounded-2xl bg-primary/10 p-3 text-primary"><MessageCircle className="h-6 w-6" /></div><p className="text-sm font-semibold">Selecione uma conversa</p><p className="mt-1 max-w-xs text-xs leading-5 text-muted-foreground">O histórico, a janela de atendimento e o campo de resposta aparecerão aqui.</p></div> : <>
            ...
            <div className="flex-1 space-y-2 overflow-y-auto p-3 sm:p-4">{isThreadLoading ? <div className="flex items-center justify-center gap-2 py-10 text-xs text-muted-foreground"><RefreshCw className="h-4 w-4 animate-spin text-primary" /> Carregando histórico...</div> : messages.length === 0 ? <p className="py-10 text-center text-xs text-muted-foreground">Nenhuma mensagem persistida nesta conversa.</p> : messages.map((message) => <div key={message.id} className={cn("flex", message.direction === "outbound" ? "justify-end" : "justify-start")}><div className={cn("max-w-[88%] rounded-2xl px-3 py-2 text-xs", message.direction === "outbound" ? "rounded-br-sm bg-primary text-primary-foreground" : "rounded-bl-sm bg-secondary text-foreground")}>
              {message.message_type === "audio" && (message.media_url ? <audio key={message.media_url} controls preload="metadata" src={message.media_url} onError={() => void loadThread(selectedId!, true)} className="w-56 max-w-full" /> : <p>{message.processing_status === "media_download_failed" ? "Áudio indisponível. Atualize para tentar novamente." : "Processando áudio..."}</p>)}
              {message.message_type === "video" && (message.media_url ? <video key={message.media_url} controls preload="metadata" playsInline src={message.media_url} onError={() => void loadThread(selectedId!, true)} className="w-64 max-w-full rounded-lg" /> : <p>{message.processing_status === "media_download_failed" ? "Vídeo indisponível. Atualize para tentar novamente." : "Processando vídeo..."}</p>)}
              {!(["audio", "video"].includes(message.message_type)) && <p className="whitespace-pre-wrap break-words">{message.body || `[${message.message_type}]`}</p>}
              {message.media_caption && <p className="mt-1 whitespace-pre-wrap break-words">{message.media_caption}</p>}
              {message.transcription && <p className="mt-1 whitespace-pre-wrap break-words text-[10px] opacity-80">Transcrição: {message.transcription}</p>}
              <div className={cn("mt-1 flex items-center justify-end gap-1 text-[9px]", message.direction === "outbound" ? "text-primary-foreground/70" : "text-muted-foreground")}><span>{formatTime(message.created_at)}</span>{message.direction === "outbound" && <CheckCheck className="h-3 w-3" />}</div></div></div>)}</div>
            <div className="border-t border-border/60 p-3 sm:p-4"><div className="mb-2 flex items-center justify-between gap-2"><div className="flex items-center gap-1.5 text-[10px] text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5 text-primary" /> {dryRun ? "Modo de teste protegido" : "Envio real habilitado"}</div><button type="button" className={cn("relative h-5 w-9 rounded-full transition-colors", dryRun ? "bg-primary" : "bg-muted")} onClick={() => { if (dryRun) { setDryRun(false); } else { setDryRun(true); } }} aria-label="Alternar modo de teste"><span className={cn("absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform", dryRun ? "left-4" : "left-0.5")} /></button></div><Textarea value={draft} onChange={(event) => setDraft(event.target.value)} disabled={!canReply || isSending} placeholder={canReply ? "Escreva uma resposta..." : "Janela fechada: selecione um template aprovado"} className="min-h-20 resize-none rounded-lg text-sm" maxLength={4096} /><div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><p className="text-[10px] leading-4 text-muted-foreground">{canReply ? "Resposta livre permitida pela janela de atendimento." : "A V4 bloqueia mensagens livres fora da janela de 24 horas e sugere template aprovado."}</p><Button onClick={() => void handleReply()} disabled={!canReply || !draft.trim() || isSending} className="w-full rounded-lg sm:w-auto"><Send className="mr-2 h-3.5 w-3.5" />{isSending ? "Enviando..." : dryRun ? "Simular resposta" : "Enviar resposta"}</Button></div></div>
          </>}
        </div>
      </CardContent>
    </Card>
