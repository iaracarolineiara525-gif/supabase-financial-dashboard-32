...
import { PersonAvatar, PersonIdentity } from "@/components/PersonIdentity";
...
import { BroadcastPanel, BroadcastQueuePanel, BroadcastReportsPanel } from "@/components/BroadcastPanels";
...
type Section = "overview" | "automation" | "campaigns" | "dispatches" | "queue" | "templates" | "contacts" | "inbox" | "reports" | "health" | "api" | "settings";
...
const navGroups: Array<{ label: string; items: NavItem[] }> = [
  { label: "Comando", items: [
    ...
    { id: "overview", label: "Dashboard", description: "Dados estruturados da operação", icon: LayoutDashboard },
  ] },
  { label: "Operação", items: [
    { id: "contacts", label: "Listas", description: "Contatos e públicos", icon: Users },
    { id: "campaigns", label: "Disparos", description: "Envio por lista ou contato", icon: Send },
    { id: "automation", label: "Fluxo de Automação", description: "Sequências automáticas com respostas", icon: Workflow },
    ...
    { id: "reports", label: "Relatórios", description: "Histórico por disparo", icon: BarChart3 },
    { id: "health", label: "Saúde da conta", description: "Qualidade e alertas", icon: ShieldCheck },
  ] },
];


// A operação não usa dados fictícios. Os contatos e campanhas aparecem somente
// quando existirem registros reais na Meta Distribuidora ou após uma importação validada.
const contacts: Contact[] = [];
const campaigns: Array<{ name: string; status: string; delivered: string; audience: string; time: string }> = [];

const sectionCopy: Record<Section, { eyebrow: string; title: string; description: string }> = {
  overview: { eyebrow: "CENTRO DE COMANDO", title: "Dashboard de dados", description: "Indicadores, funil de entrega e histórico calculados a partir dos registros reais do servidor." },
  automation: { eyebrow: "AUTOMAÇÃO DE MENSAGENS", title: "Fluxo de Automação", description: "Crie sequências com mensagens, esperas e verificação de resposta, com caminhos separados para quem respondeu e quem não respondeu." },
  dispatches: { eyebrow: "HISTÓRICO DE ENVIOS", title: "Disparos", description: "Veja lista, data, hora, destino e status de cada disparo registrado pelo servidor." },
  campaigns: { eyebrow: "ENVIO EM MASSA", title: "Disparos", description: "Escolha a lista ou o contato, selecione o template aprovado e acompanhe o progresso em tempo real." },
  queue: { eyebrow: "OPERAÇÃO POR MENSAGEM", title: "Fila e entregas", description: "Status individual de cada contato: pendente, enviando, enviado, entregue ou falhou." },
  ...
  contacts: { eyebrow: "BASE DE RELACIONAMENTO", title: "Listas", description: "Suba uma lista com nome e telefone ou cadastre contatos individuais com padronização automática." },
  ...
  reports: { eyebrow: "VISÃO DE DESEMPENHO", title: "Relatórios", description: "Histórico por disparo com data, hora, lista, template e totais enviados." },
  ...
  settings: { eyebrow: "ADMINISTRAÇÃO META DISTRIBUIDORA", title: "Configurações", description: "Gerencie perfis, regras, trilha de auditoria e preferências da operação." },
};
...
function MetaTemplatesPanel() {
  ...
  const handleSubmit = async () => {
    ...
    const confirmed = window.confirm("Enviar este template para análise da Meta? A Meta fará a aprovação; o sistema não marca templates como aprovados localmente.");
    if (!confirmed) return;
      ...
      <CardContent className="grid gap-4 p-5 pt-1 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-3 rounded-xl border border-border/60 bg-secondary/20 p-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              value={headerType}
              onChange={(event) => { setHeaderType(event.target.value as typeof headerType); setHeaderFile(null); setHeaderError(null); setHeaderProgress(0); }}
            ...
            {["image", "video", "document"].includes(headerType) && (
              <div className="space-y-1.5">
                <input
                  type="file"
                  accept={acceptAttribute(headerType as MediaKind)}
                  onChange={(event) => { setHeaderFile(event.target.files?.[0] || null); setHeaderError(null); }}
                  className="w-full rounded-lg border border-input bg-background p-2 text-xs text-foreground"
                />
                {headerType === "video" && <p className="text-[11px] text-muted-foreground">Vídeo MP4 ou 3GP até 16 MB, com vídeo H.264 e áudio AAC.</p>}
                {headerProgress > 0 && headerProgress < 100 && <div className="h-1.5 w-full overflow-hidden rounded-full bg-secondary"><div className="h-full bg-primary transition-all" style={{ width: `${headerProgress}%` }} /></div>}
                {headerError && <p className="text-[11px] text-destructive">{headerError}</p>}
              </div>
            )}
            <p className="text-[11px] text-muted-foreground">A Meta não aceita áudio como cabeçalho de template — apenas texto, imagem, vídeo ou documento.</p>
          </div>
          ...
          <p className="text-[10px] leading-4 text-muted-foreground">O status só muda após a análise da Meta. Em produção, a submissão é enviada à Meta para análise e o status aparece após a atualização.</p>
        </div>
...
export default function Index() {
  ...
  const [broadcastList, setBroadcastList] = useState<string | null>(null);
  ...
  const showStatusToast = (title: string, description: string) => toast({ title, description });
  ...
  const handleTestConnection = async () => {
    ...
    showStatusToast("Conexão validada", "A Meta respondeu e a conta real está acessível. Nenhuma mensagem foi enviada.");
  };
  ...
  const loadApprovedTemplates = async () => {
    ...
    if (error || !data?.ok) {
      setApprovedError("Não foi possível carregar os templates aprovados agora.");
      return;
    }
    setApprovedError(null);
    const list = (Array.isArray(data.templates) ? data.templates : []).filter((template: MetaTemplate) => String(template.status || "").toUpperCase() === "APPROVED");
  ...
  const selectedTemplate = useMemo(() => approvedTemplates.find((template) => `${template.name}|${template.language}` === selectedTemplateKey) || null, [approvedTemplates, selectedTemplateKey]);
  const selectedTemplateBody = selectedTemplate?.components?.find((component) => component.type?.toUpperCase() === "BODY")?.text || "";
  const previewText = selectedTemplateBody
    ? selectedTemplateBody.replace(/\{\{\s*(\d+)\s*\}\}/g, (_match, position) => templateVariables[Number(position) - 1] || `{{${position}}}`)
    : "Selecione um template aprovado para visualizar a prévia.";
  const selectedTemplateVariableCount = useMemo<number>(() => {
    const matches: string[] = selectedTemplateBody.match(/\{\{\s*(\d+)\s*\}\}/g) ?? [];
  ...
  const handleRealSend = async () => {
    ...
    if (!selectedTemplate?.name) {
      toast({ title: "Escolha um template aprovado", description: "Fora da janela de 24h o WhatsApp só aceita template aprovado pela Meta.", variant: "destructive" });
      return;
    }
    const parameters = Array.from({ length: selectedTemplateVariableCount }, (_, index) => (templateVariables[index] || "").trim());
    if (parameters.some((value) => !value)) {
      toast({ title: "Preencha as variáveis", description: `Este template exige ${selectedTemplateVariableCount} variável(is).`, variant: "destructive" });
      return;
    }
    const confirmed = window.confirm(`Confirma o envio REAL da campanha "${campaignName}" usando o template "${selectedTemplate.name}" para 1 destinatário?`);
    if (!confirmed) return;
    setIsSending(true);
    ...
    showStatusToast("Solicitação aceita pela Meta", "Acompanhe o status real pelo webhook.");
  };
  ...
  return (
    <div className={`relative min-h-[100dvh] overflow-hidden ${theme === "dark" ? "brand-gradient-radial" : "brand-gradient-radial-light"} brand-grid`}>
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-50 flex h-dvh w-[min(86vw,280px)] flex-col border-r border-border/70 bg-card/95 shadow-2xl shadow-black/10 backdrop-blur-md transition-transform duration-300 lg:z-40 lg:w-56 lg:translate-x-0 xl:w-60", mobileNavOpen ? "translate-x-0" : "-translate-x-full", sidebarOpen ? "lg:w-56 xl:w-60" : "lg:w-16 xl:w-16")}>
        <div className="border-b border-border/60 p-3"><div className={cn("flex items-center", sidebarOpen ? "gap-3" : "justify-center")}><PersonIdentity className={cn("flex-1", !sidebarOpen && !mobileNavOpen && "justify-center")} showLabel={sidebarOpen || mobileNavOpen} photoClassName="h-10 w-10 rounded-xl" /></div></div>
        <div className={cn("border-b border-border/60 p-3", sidebarOpen ? "" : "flex justify-center")}>
          {(sidebarOpen || mobileNavOpen) ? <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-2"><p className="truncate text-sm font-semibold text-foreground">Meta Distribuidora</p><p className="truncate text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Cliente ativo</p></div> : <PersonAvatar className="h-9 w-9 rounded-xl" />}
        </div>
        <nav className="flex-1 space-y-2 overflow-y-auto p-2">{navGroups.map((group) => <div key={group.label} className="space-y-0.5">{(sidebarOpen || mobileNavOpen) && <p className="px-2.5 pt-1 text-[9px] font-bold uppercase tracking-[0.18em] text-muted-foreground/70">{group.label}</p>}{group.items.map((item) => <button key={item.id} type="button" title={item.description} onClick={() => { setActiveSection(item.id); setMobileNavOpen(false); }} className={cn("group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-all duration-200", activeSection === item.id ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "text-muted-foreground hover:bg-secondary hover:text-foreground", !sidebarOpen && !mobileNavOpen && "justify-center px-2")}><item.icon className="h-5 w-5 shrink-0" />{(sidebarOpen || mobileNavOpen) && <span className="min-w-0 truncate text-sm font-semibold">{item.label}</span>}</button>)}</div>)}</nav>
      ...
      <main className={cn("relative z-10 min-h-screen min-w-0 transition-all duration-300", sidebarOpen ? "lg:ml-56 xl:ml-60" : "lg:ml-16")}>
        <header className="sticky top-0 z-30 border-b border-border/70 bg-card/90 shadow-sm backdrop-blur-xl"><div className="flex items-center justify-between border-b border-border/50 px-4 py-2.5 lg:hidden"><Button variant="outline" size="icon" className="h-9 w-9 rounded-xl bg-transparent" aria-label="Abrir menu" onClick={() => setMobileNavOpen(true)}><Menu className="h-4 w-4" /></Button><PersonAvatar className="h-8 w-8 rounded-lg" /><ThemeToggle /></div><div className="flex flex-col gap-2.5 px-4 py-3 sm:px-5 lg:px-6"><div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between"><div><div className="mb-1 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.22em] text-primary"><span className="h-1.5 w-1.5 rounded-full bg-primary" /> {copy.eyebrow}</div><h1 className="max-w-3xl text-xl font-semibold leading-tight tracking-[-0.05em] sm:text-2xl lg:text-3xl">{copy.title}</h1><p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">{copy.description}</p></div><div className="hidden flex-wrap items-center gap-2 lg:flex"><div className="flex items-center gap-2 rounded-xl border border-border/70 bg-background/70 px-3 py-2"><PersonIdentity className="max-w-[13rem]" photoClassName="h-8 w-8 rounded-lg" /></div><ThemeToggle /><Button variant="outline" size="icon" className="rounded-xl bg-transparent" onClick={() => showStatusToast("Atualizado", "Consulte as funções conectadas para atualizar os dados reais.")}><RefreshCw className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="rounded-xl bg-transparent"><Bell className="h-4 w-4" /></Button></div></div><div className="flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground"><span className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-secondary/50 px-2.5 py-1"><LockKeyhole className="h-3 w-3" /> Ambiente protegido</span></div></div></header>
        <div className="p-3 sm:p-4 lg:p-5">{activeSection === "overview" && <DashboardPanel onNavigate={(section) => setActiveSection(section)} />}{activeSection === "campaigns" && <BroadcastPanel initialList={broadcastList} />}{activeSection === "automation" && <AutomationPanel />}{activeSection === "dispatches" && <DispatchPanel />}{activeSection === "queue" && <BroadcastQueuePanel />}{activeSection === "templates" && renderTemplates()}{activeSection === "contacts" && <ListsPanel onUseList={(listName) => { setBroadcastList(listName); setActiveSection("campaigns"); }} />}{activeSection === "inbox" && <InboxPanel />}{activeSection === "reports" && <BroadcastReportsPanel />}{activeSection === "health" && <AccountHealthPanel />}{activeSection === "api" && renderApi()}{activeSection === "settings" && <SettingsPanel operatorRole={operator?.role || "Sessão ativa"} />}</div>
        <footer className="border-t border-border/60 px-3 py-2 text-center text-[11px] text-muted-foreground sm:px-5 lg:px-7"><span>Meta Distribuidora · Operação de mensagens</span><span className="mx-2 text-border">•</span><span>Dados e consentimentos sob controle</span></footer>
      </main>
    </div>
  );
}
